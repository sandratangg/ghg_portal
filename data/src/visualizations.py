...
def plot_top_sectors(df, top_n=10):
    ...
    fig = px.bar(
        ...
        labels={
            "sum_millions": "Total Emissions (Million Metric Tons)",
...
def plot_state_map(df):
    ...
    fig.update_layout(geo=dict(showlakes=True, lakecolor="lightblue"), height=500)
...
def plot_yearly_trends(df):
    ...
    sector_yearly = (
        df[df["industry_sector_clean"].isin(top_sectors)]
        .groupby(["reporting_year", "industry_sector_clean"])[
    ...
    for i, sector in enumerate(top_sectors):
        ...
        fig.add_trace(
            go.Scatter(
                x=sector_data["reporting_year"],
                y=sector_data["total_ghg_emissions_tonnes"] / 1_000_000,
                mode="lines+markers",
                name=sector[:15],
...
def plot_outliers(df, threshold_percentile=95):
    ...
    fig.add_trace(
        go.Histogram(
            x=np.log10(outliers["total_ghg_emissions_tonnes"] + 1),
            nbinsx=20,
            name="Outliers",
            opacity=0.8,
        ),
    ...
    fig.add_trace(
        go.Bar(
            x=top_outliers["total_ghg_emissions_tonnes"] / 1_000_000,
    ...
    fig.add_trace(
        go.Bar(
            x=outliers_by_state["state"],
            y=outliers_by_state["count"],
    ...
    outliers_by_sector = outliers_by_sector.sort_values("count", ascending=False).head(
        8
    )
    fig.add_trace(
        go.Bar(
            x=outliers_by_sector["count"],
            y=outliers_by_sector["industry_sector_clean"],
            orientation="h",
            name="Outliers by Sector",
        ),
        row=2,
        col=2,
    )
...
def plot_model_performance(y_true, y_pred, model_name="Model"):
    ...
    fig.add_trace(
        go.Scatter(
            x=y_pred, y=residuals, mode="markers", name="Residuals", opacity=0.6
        ),
        row=1,
        col=2,
    )
